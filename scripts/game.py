...
class Game():
    ...
    def __init__(self, board_width, board_height, should_use_ai, ai_genes, background=(0, 0, 0), line_color=(0, 255, 0), line_thickness=3, shape_thickness=2):
        ...
        self._PLAYER_SHAPES= {}
        self._PLAYER_SHAPES[Game.PLAYER1] = Game.X
        self._PLAYER_SHAPES[Game.PLAYER2] = Game.O
    ...
    def _draw_shape(self, screen):
        ...
        if self._PLAYER_SHAPES[self._current_player] == Game.X:
        ...
        elif self._PLAYER_SHAPES[self._current_player] == Game.O:
    ...
    def _check_if_game_over(self):
        ...
        for i in range(3):
            if (sum([self._player_1_filled[i][j] for j in range(3)]) > 2) \
                ...
                or (sum([self._player_2_filled[j][i] for j in range(3)]) > 2):
                self._is_over = True
                self._winning_player = Game.PLAYER2
                return(True)
        if (self._player_1_filled[0][0] + self._player_1_filled[1][1] + self._player_1_filled[2][2] > 2) \
            or (self._player_1_filled[0][2] + self._player_1_filled[1][1] + self._player_1_filled[2][0] > 2):
            self._is_over = True
            self._winning_player = Game.PLAYER1
            return(True)
        if (self._player_2_filled[0][0] + self._player_2_filled[1][1] + self._player_2_filled[2][2] > 2) \
            or (self._player_2_filled[0][2] + self._player_2_filled[1][1] + self._player_2_filled[2][0] > 2):
            self._is_over = True
            self._winning_player = Game.PLAYER2
            return(True)
